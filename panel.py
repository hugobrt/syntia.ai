...
RSS_PAUSE_FILE = os.path.join(DATA_DIR, "rss_paused.json")  # ← AJOUT PAUSE RSS
...
def log_admin_action(user_id: int, action: str, details: str = ""):
    ...
    logs.append({"timestamp": datetime.now().isoformat(), "user_id": user_id, "action": action, "details": details})
    if len(logs) > 1000:
        logs = logs[-1000:]
...
def get_server_config(guild_id: int) -> dict:
    ...
    if str(guild_id) not in configs:
        configs[str(guild_id)] = {
            "ticket_category": None, "ticket_logs": None, "suggestions_channel": None,
            "logs_channel": None, "welcome_channel": None, "goodbye_channel": None,
            "level_up_channel": None, "autorole": None, "prefix": "!", "language": "fr"
        }
...
try:
    ...
    USE_RSS_DB = False
    logger.warning("RSS: Mode JSON fallback (bot2 non importe)")
    def get_rss_feeds():
...
def add_reminder(user_id: int, channel_id: int, message: str, remind_at: datetime, guild_id: int) -> dict:
    ...
    reminder = {"id": len(reminders) + 1, "user_id": user_id, "channel_id": channel_id,
                "guild_id": guild_id, "message": message, "remind_at": remind_at.isoformat(),
                "created_at": datetime.now().isoformat(), "done": False}
...
def get_due_reminders() -> list:
    reminders = get_reminders()
    now = datetime.now()
    return [r for r in reminders if not r.get("done") and now >= datetime.fromisoformat(r["remind_at"])]
...
def create_backup(guild_id: int, user_id: int) -> str:
    ...
    backup_data = {"guild_id": guild_id, "created_by": user_id, "created_at": datetime.now().isoformat(),
                   "server_config": config, "rss_feeds": get_rss_feeds() if not USE_RSS_DB else [],
                   "reminders_count": len(get_reminders())}
    filename = f"backup_{guild_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    save_json(os.path.join(BACKUPS_DIR, filename), backup_data)
    log_admin_action(user_id, "backup_created", filename)
    return filename

def list_backups() -> list:
    try:
        return sorted([f for f in os.listdir(BACKUPS_DIR) if f.endswith('.json')], reverse=True)[:10]
    except:
        return []

def restore_backup(filename: str, guild_id: int) -> bool:
    data = load_json(os.path.join(BACKUPS_DIR, filename))
    if not data:
        return False
    configs = load_json(CONFIG_FILE, {})
    configs[str(guild_id)] = data.get("server_config", {})
    save_json(CONFIG_FILE, configs)
    return True
...
def get_admin_logs(limit: int = 50) -> list:
    return load_json(LOGS_FILE, [])[:limit]
...
def embed_to_dict(embed: discord.Embed) -> dict:
    data = {"title": embed.title, "description": embed.description,
            "color": hex(embed.color.value)[2:] if embed.color else None, "url": embed.url,
...
def get_user_info_embed(user: discord.Member) -> discord.Embed:
    status_colors = {discord.Status.online: 0x57F287, discord.Status.idle: 0xFEE75C,
                     discord.Status.dnd: 0xED4245, discord.Status.offline: 0x747F8D}
    ...
    status_map = {discord.Status.online: "En ligne", discord.Status.idle: "Absent",
...
class EmbedCustomizeView(discord.ui.View):
    ...
    async def export_json(self, i, b):
        ...
        if len(json_str) > 1900:
            await i.response.send_message("JSON trop long:", file=discord.File(fp=json_str.encode(), filename="embed.json"), ephemeral=True)
        else:
...
class RSSAddModal(discord.ui.Modal, title="Ajouter Flux RSS"):
    ...
    async def on_submit(self, i: discord.Interaction):
        ...
        if not url.startswith(("http://", "https://")):
            await i.followup.send("URL doit commencer par http:// ou https://", ephemeral=True); return
...
class ReminderModal(discord.ui.Modal, title="Creer un Rappel"):
    ...
    async def on_submit(self, i: discord.Interaction):
        ...
        if minutes <= 0:
            await i.response.send_message("Format invalide ! Ex: 30m, 2h, 1d", ephemeral=True); return
...
class EmbedTemplateEditModal(discord.ui.Modal, title="Modifier Template"):
    ...
    async def on_submit(self, i: discord.Interaction):
        try:
            ...
            ok = bot_save(self.template_name, self.title_input.value or None, self.desc_input.value or None,
                          self.color_input.value, self.footer_input.value or None, None, None, None, [], i.user.id)
        except ImportError:
            ok = save_embed_template(self.template_name, {"title": self.title_input.value, "description": self.desc_input.value,
                                                           "color": self.color_input.value, "footer": self.footer_input.value})
            ok = True
        if ok:
...
# ====================================================
# SELECTEURS UTILISATEURS / SALONS
# ====================================================
...
class ChanSel(discord.ui.View):
    ...
    async def s(self, i, s):
        ...
        if self.a == "embed":
            ...
            log_admin_action(i.user.id, "nuke", str(c.id))
            await i.response.edit_message(
                content=f"✅ **Nuke effectue !** Nouveau salon : {nc.mention}\n"
                        f"🔄 Liste mise a jour — renuke ou autre action :",
                view=ChanSel(self.a)
            )
        elif self.a == "lock":
...
# ====================================================
# PATCH CORRECTIF — Config avec ChannelSelect natif
# ====================================================

class ConfigChannelSelect(discord.ui.ChannelSelect):
    def __init__(self, config_key: str, label_name: str, ch_types: list):
        super().__init__(
            placeholder=f"📌 Choisir le salon {label_name}...",
    ...
    async def callback(self, i: discord.Interaction):
        ...
        set_server_config(i.guild.id, self.config_key, channel.id)
        log_admin_action(i.user.id, f"config_{self.config_key}", str(channel.id))
        await i.response.send_message(f"✅ **{self.label_name}** configure sur {channel.mention} !", ephemeral=True)

class ConfigCategorySelect(discord.ui.ChannelSelect):
    def __init__(self):
        super().__init__(
            placeholder="📂 Choisir la categorie tickets...",
            channel_types=[discord.ChannelType.category],
            min_values=1, max_values=1
        )
    async def callback(self, i: discord.Interaction):
        cat = self.values[0]
        set_server_config(i.guild.id, "ticket_category", cat.id)
        log_admin_action(i.user.id, "config_tickets", str(cat.id))
        await i.response.send_message(f"✅ **Categorie tickets** → **{cat.name}** !", ephemeral=True)

def make_channel_select_view(config_key: str, label: str, ch_types: list = None) -> discord.ui.View:
    if ch_types is None:
        ch_types = [discord.ChannelType.text]
    view = discord.ui.View(timeout=60)
    view.add_item(ConfigChannelSelect(config_key, label, ch_types))
    return view

def make_category_select_view() -> discord.ui.View:
    view = discord.ui.View(timeout=60)
...
class ConfigView(discord.ui.View):
    ...
    async def config_tickets(self, i: discord.Interaction, b):
        await i.response.send_message("📂 Selectionne la **categorie** pour les tickets :", view=make_category_select_view(), ephemeral=True)

    @discord.ui.button(label="Suggestions", style=discord.ButtonStyle.primary, row=0, emoji="💡", custom_id="config_suggestions")
    async def config_suggestions(self, i: discord.Interaction, b):
        await i.response.send_message("💡 Selectionne le salon **Suggestions** :", view=make_channel_select_view("suggestions_channel", "Suggestions"), ephemeral=True)

    @discord.ui.button(label="Logs", style=discord.ButtonStyle.primary, row=0, emoji="📜", custom_id="config_logs")
    async def config_logs(self, i: discord.Interaction, b):
        await i.response.send_message("📜 Selectionne le salon **Logs** :", view=make_channel_select_view("logs_channel", "Logs"), ephemeral=True)

    @discord.ui.button(label="Welcome", style=discord.ButtonStyle.primary, row=1, emoji="👋", custom_id="config_welcome")
    async def config_welcome(self, i: discord.Interaction, b):
        await i.response.send_message("👋 Selectionne le salon **Welcome** :", view=make_channel_select_view("welcome_channel", "Welcome"), ephemeral=True)

    @discord.ui.button(label="Goodbye", style=discord.ButtonStyle.primary, row=1, emoji="👋", custom_id="config_goodbye")
    async def config_goodbye(self, i: discord.Interaction, b):
        await i.response.send_message("👋 Selectionne le salon **Goodbye** :", view=make_channel_select_view("goodbye_channel", "Goodbye"), ephemeral=True)

    @discord.ui.button(label="Level Up", style=discord.ButtonStyle.primary, row=1, emoji="⬆️", custom_id="config_levelup")
    async def config_levelup(self, i: discord.Interaction, b):
        await i.response.send_message("⬆️ Selectionne le salon **Level Up** :", view=make_channel_select_view("level_up_channel", "Level Up"), ephemeral=True)

    @discord.ui.button(label="Voir Config", style=discord.ButtonStyle.success, row=2, emoji="📋", custom_id="config_view")
    async def view_config(self, i: discord.Interaction, b):
        config = get_server_config(i.guild.id)
        embed = discord.Embed(title="⚙️ Configuration Serveur", color=0x5865F2, timestamp=datetime.now())
        keys = {"ticket_category": "🎫 Tickets (categorie)", "suggestions_channel": "💡 Suggestions",
                "logs_channel": "📜 Logs", "welcome_channel": "👋 Welcome",
                "goodbye_channel": "👋 Goodbye", "level_up_channel": "⬆️ Level Up"}
...
# ====================================================
# RSS VIEW — avec bouton Pause/Reprise
# ====================================================
...
class RSSView(discord.ui.View):
    ...
    async def list_rss(self, i, b):
        ...
        embed = discord.Embed(
            title=f"Flux RSS Actifs {'⏸ EN PAUSE' if paused else '▶️ Actif'}",
            color=0xFF6600 if paused else 0x0055ff,
            description=f"**{len(feeds)}** flux configure(s)"
        )
        for f in feeds[:10]:
            if isinstance(f, dict):
    ...
    @discord.ui.button(label="⏸ Pause RSS", style=discord.ButtonStyle.secondary, row=2, emoji="⏸️", custom_id="rss_pause_toggle")
    async def pause_rss(self, i: discord.Interaction, b):
        paused = is_rss_paused()
        set_rss_paused(not paused)
        if not paused:
            # Mise en pause → embed bleu dans le salon
            embed = discord.Embed(
                title="⏸  FEED RSS EN PAUSE",
                description="Les actualités automatiques sont **suspendues**.\nAppuie sur **▶️ Reprendre RSS** dans le panel pour les réactiver.",
                color=0x0055ff
            )
            embed.set_footer(text=f"Mis en pause par {i.user.display_name}")
            embed.timestamp = datetime.now()
            await i.channel.send(embed=embed)
            await i.response.send_message("⏸️ RSS mis en **pause** ! Embed envoyé dans le salon.", ephemeral=True)
        else:
            await i.response.send_message("▶️ RSS **réactivé** ! Les articles reprendront au prochain cycle (30 min).", ephemeral=True)
        log_admin_action(i.user.id, "rss_pause", "pause" if not paused else "reprise")

    @discord.ui.button(label="Retour", style=discord.ButtonStyle.secondary, row=2, emoji="🔙", custom_id="rss_back")
    async def back(self, i, b):
        embed = discord.Embed(title="INFINITY PANEL V46", color=0x2b2d31)
        await i.response.edit_message(embed=embed, view=MainPanelView())
...
class MainPanelView(discord.ui.View):
    ...
    async def rss(self, i, b):
        paused = is_rss_paused()
        embed = discord.Embed(
            title=f"Gestion RSS {'⏸ EN PAUSE' if paused else ''}",
            color=0xFF6600 if paused else 0x0055ff
        )
    ...
    async def market_admin(self, i, b):
        if not i.user.guild_permissions.administrator:
            await i.response.send_message("Admin requis !", ephemeral=True); return
        await i.response.send_message(embed=discord.Embed(title="Market Admin", color=0xFFD700), view=MarketAdminView(), ephemeral=True)
    ...
    async def nuke(self, i, b):
        await i.response.send_message("⚠️ ATTENTION — Quel salon nuker ?", view=ChanSel("nuke"), ephemeral=True)
    ...
    try:
        ...
        for reminder in due:
            ...
            if channel:
                ...
                embed = discord.Embed(title="⏰ Rappel !", description=f"{mention}\n\n{reminder['message']}",
...
class AdminPanel(commands.Cog):
    ...
    async def setup_panel(self, interaction: discord.Interaction):
        embed = discord.Embed(title="INFINITY PANEL V46", description="**PANEL ADMIN**\n\n",
                              color=0x2b2d31, timestamp=datetime.now())
        await interaction.channel.send(embed=embed, view=MainPanelView())
        await interaction.response.send_message("Panel V46 deploye !", ephemeral=True)
