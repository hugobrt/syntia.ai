...
class ChanSel(discord.ui.View):
    ...
    async def s(self, i: discord.Interaction, s: discord.ui.ChannelSelect):
        ...
        # --- FIX TICKET INSTALLATION (Permission Check) ---
        elif self.a=="ticket": 
            await i.response.defer(ephemeral=True)
            ...
            try:
                ...
                await i.followup.send(f"✅ Ticket installé avec succès dans {c.mention}", ephemeral=True)
            except Exception as e: 
                await i.followup.send(f"❌ Erreur technique : {e}", ephemeral=True)
...
# ====================================================
# 4. DASHBOARD - NAVIGATION SECURISEE
# ====================================================

# VUE SECONDAIRE : GESTION BOT
class BotControlView(discord.ui.View):
    ...
    async def maint(self, i, b):
        await i.client.change_presence(status=discord.Status.dnd, activity=discord.Game(name="⚠ MAINTENANCE"))
    ...
    async def custom(self, i, b): await i.response.send_modal(StatusModal())

    # RETOUR (ID SECURISE)
    @discord.ui.button(label="RETOUR", style=discord.ButtonStyle.secondary, row=2, emoji="🔙", custom_id="nav:main_menu")
    async def back(self, i, b): pass # Géré par on_interaction
...
# ====================================================
# 6. SETUP & LISTENERS (CERVEAU CENTRAL)
# ====================================================
class AdminPanel(commands.Cog):
    ...
    async def on_ready(self):
        ...
        print("🛡️ Panel V28 (FINAL PATCHED) Ready.")
    ...
    async def on_interaction(self, i: discord.Interaction):
        ...
        # --- NAVIGATION FORCEE ---
        if cid == "nav:bot_menu":
            await i.response.edit_message(embed=discord.Embed(title="🤖 GESTION BOT", description="Contrôle d'état système.", color=0xE74C3C), view=BotControlView())
        elif cid == "nav:main_menu":
            await i.response.edit_message(embed=discord.Embed(title="🛡️ PANEL V28", color=0x2b2d31), view=MainPanelView())
        ...
        # --- TICKET (OUVERTURE) ---
            ...
            await i.response.defer(ephemeral=True)
            g=i.guild
            # Verification permissions Admin du BOT
    ...
    async def setup_panel(self, i: discord.Interaction):
        await i.channel.send(embed=discord.Embed(title="🛡️ PANEL V28 (ULTIMATE)", color=0x2b2d31), view=MainPanelView())
